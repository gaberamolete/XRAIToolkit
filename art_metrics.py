...
def visualisation(pdtp_art, shapr_art, pdtp_threshold_value, shapr_threshold_value):
    ...
    """
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=np.arange(1,50), y=pdtp_art[0][:50] / 50, mode='markers',
                            marker=dict(color = 'blue'), name='pdtp'))

    fig.add_trace(go.Scatter(x=np.arange(1,50), y=[np.mean(pdtp_art[0][:50] / 50)]*50, mode='lines', name='pdtp average'))
